...
for device in paired:
    if(bl.remove(device["mac_address"])):
        print("success")
if not paired:
    print("None")
    print()
...
for index, device in enumerate(devs):
    ...
    print("Addr:", device["mac_address"])
    print()
if not devs:
    print("None")
    print()
pair_id = None
while pair_id is None:
    try:
        pair_id = int(input("Enter device no. to pair: "))
    except ValueError:
        print("Enter integer")

pair_mac = devs[pair_id]["mac_address"]
bl.pair(pair_mac)
print("Pairing successful")

"""
bl.child.send("trust " + pair_mac)
cmd = "sudo rfcomm bind 0 "+pair_mac
